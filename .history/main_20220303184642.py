...
class RLTrainer(object):
    def __init__(self,args,device):
        ...
        self.rollouts.obs[0].copy_(torch.from_numpy(state))
    ...
    def collect_rollouts(self):
        with torch.no_grad():
            for step in range(self.env_steps_per_update):
                ...
                self.rollouts.insert(torch.from_numpy(state).to(self.device), recurrent_hidden_states.to(self.device), action, action_log_prob,
