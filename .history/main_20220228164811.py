...
class RLTrainer(object):
    ...
    def collect_rollouts(self):
        with torch.no_grad():
            for step in range(self.env_steps_per_update):
                ...
                # next_state, reward, dine, info = self.envs.step(action.cpu().numpy())
    ...
    def train(self):
        for i in range(self.num_updates):
            ...
            if i % self.args.log_interval == 0 and len(episode_rewards) > 1:
            ...
            if (self.args.eval_interval is not None and len(episode_rewards) > 1
                    and i % self.args.eval_interval == 0):
                obs_rms = utils.get_vec_normalize(self.envs).obs_rms
                self.evaluate(self.algo.actor_critic, obs_rms, self.args.env_name, self.args.seed,
