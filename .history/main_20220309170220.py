...
class RLTrainer(object):
    ...
    def collect_rollouts(self):
        with torch.no_grad():
            for step in range(self.env_steps_per_update):
                self.rollouts.obs[step] = self.algo.actor_critic.normalize_state(self.rollouts.obs[step], update=True)  # normalize state
    ...
    def train(self):
        ...
        for i in range(self.num_updates):
            ...
            with torch.no_grad():
                self.rollouts.obs[-1] = self.algo.actor_critic.normalize_state(self.rollouts.obs[-1], update=True)  # normalize state
