...
class RLTrainer(object):
    def __init__(self,args,device):
        ...
        eval_envs = [make_env(args.env_name) for _ in range(args.num_processes)]
        self.eval_envs = SubprocVecEnv(eval_envs)
    ...
    def train(self):
        for i in range(self.num_updates):
            ...
            #TODO
            value_loss_epoch, action_loss_epoch, dist_entropy_epoch = self.algo.update(self.rollouts)
    ...
    def test_env(self):
        ...
        while not done:
            ...
            with torch.no_grad():
                ...
                deterministic=True)
                next_state, reward, done, _ = self.eval_envs.step(action.cpu().numpy())
                state = next_state
                total_reward += reward
                # cnt += 1
        # print(cnt)
