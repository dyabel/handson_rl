...
class RLTrainer(object):
    def __init__(self,args,device):
        ...
        actor_critic = ActorCritic(state_shape.shape,action_shape)
    ...
    def collect_rollouts(self):
        for step in range(self.env_steps_per_update):
            ...
            #      for info_ in info])
            self.rollouts.insert(torch.from_numpy(next_state), torch.from_numpy(recurrent_hidden_states), torch.from_numpy(action), torch.from_numpy(action_log_prob),
               torch.from_numpy(value), torch.from_numpy(reward), torch.from_numpy(mask))
