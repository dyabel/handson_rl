    ...
    def normalize_state(self, state, update=True):
        ...
        if update:
            if len(state.size()) >= 1:  # If we get a single state vector
                ...
                self.welford_state_mean += (state.mean((-3,-2)) - state_old) / self.welford_state_n
                self.welford_state_mean_diff += (state.mean((-3,-2)) - self.welford_state_mean) * (state.mean((-2,-1)) - state_old)
                self.welford_state_n += 1
            else:
                raise RuntimeError
